...
const int D0 = 17; // R_MOTOR
const int D1 = 16; // R_MOTOR
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len)
{
    ...
    digitalWrite(data.pin1, data.on_off);
    digitalWrite(data.pin2, data.on_off);
    digitalWrite(data.pin3, data.on_off);
    digitalWrite(data.pin4, data.on_off);
}
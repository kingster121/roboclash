...
{                                                // called by void loop(), which isDriving = true
    ...
    if (deltaAngle != 0)
    {
        ...
        // right
        //(rightSpeed, rightSpeedVal);
        //(leftSpeed, leftSpeedVal);
    }
}
...
int changeSpeed(int motorSpeed, int increment)
{
    ...
    { // to prevent motorSpeed from exceeding 255, which is a problem when using //
        motorSpeed = maxSpeed;
    }
...
{ // drives the car forward, assuming leftSpeedVal and rightSpeedVal are set high enough
  //(right1, HIGH); //the right motor rotates FORWARDS when right1 is HIGH and right2 is LOW
  //(right2, LOW);
  //(left1, HIGH);
  //(left2, LOW);
}
